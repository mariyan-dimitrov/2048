...
const RouterContextProvider = ({ children }) => {
  ...
  const goBack = useCallback(() => {
    const { previousPathname, isLogged, routes } = getStoreMe(
      "previousPathname",
      "isLogged",
      "routes"
    );
    ...
    if (
      isLogged &&
      [routes.login, routes.register, routes.resetPassword, routes.forgotPassword].includes(
        previousPathname
      )
    ) {
  ...
  return (
    <RouterContext.Provider value={{ goBack, pushRoute, updateQuery }}>
      {children}
